...
class ScoreInfer(object):
    ...
    def get_rank_score(
        ...
        tail=None,
        user_set_tail=None,
        exec_mode: str = "batch_head",
    ):
        """
        Like topK with exec_mode='batch_head', but instead of returning the top-k,
        finds the position of `user_set_tail` in the full sorted list of scores for
        (head, rel, *) and returns its rank, its score, and the max score.

        Returns:
            rank (int), user_score (float), max_score (float)
        """
        # 1) validate inputs
        if exec_mode != "batch_head":
            raise NotImplementedError("get_rank_score only supports exec_mode='batch_head'")
        if head is None or len(head) != 1:
            raise ValueError("`head` must be a length-1 list or array")
        if rel is None or len(rel) != 1:
            raise ValueError("`rel` must be a length-1 list or array")
        if user_set_tail is None or len(user_set_tail) != 1:
            raise ValueError("`user_set_tail` must be a length-1 list or array")

        # 2) build tensors (default tail→all entities)
        head_ids = F.tensor(head)
        rel_ids  = F.tensor(rel)
        if tail is None:
            tail_ids = F.arange(0, self.model.num_entity)
        else:
            tail_ids = F.tensor(tail)
