...
async def send_welcome_email(email_to: EmailStr):
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <p>Dear User,</p>
                <p>Welcome to EvoAge! We are thrilled to have you on board.</p>
                <p>EvoAge is a platform designed to help you explore and understand complex knowledge graphs. We hope you find it useful and insightful.</p>
                <p>If you have any questions or need assistance, please don't hesitate to reach out to our support team.</p>
                <p>Best regards,<br><strong>The EvoAge Team</strong></p>
            </div>
            <div class="footer">
                <p>&copy; {current_year} EvoAge. All rights reserved.</p>
...
async def send_password_reset_email(email_to: EmailStr, token: str):
    ...
    logo_path = Path(__file__).parent.parent / "data" / "logo.png"
    logo_cid = "logo_reset"  # Use a new CID

    logger.debug(f"Preparing to send password reset email to {email_to}")

    # --- Get Frontend URL and build reset link ---
    try:
        # This MUST be set in your environment configuration
        frontend_url = CONFIG.FE.FRONTEND_URL.rstrip('/')
        ...
        # This is a server configuration error, so we raise it
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Email server is not configured correctly (missing FRONTEND_URL).",
        )
    
    # --- Create HTML Body (re-using existing styles) ---
    body = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                ...
                <p>If you made this request, please click the button below to set a new password. This link is valid for 15 minutes.</p>
    ...
    # --- Prepare Attachments (same logic as other functions) ---
    attachments = []
    if logo_path.is_file():
        attachments.append(
            {
        ...
        logger.warning(
            f"Logo file not found at {logo_path}. Reset email will be sent without logo."
        )
    ...
    try:
        ...
        # Re-raise as HTTPException, following the pattern in send_welcome_email
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
