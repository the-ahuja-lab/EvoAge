...
def show_microservices_page():
    ...
    def dlg_hypothesis():
        ...
        if st.button("Run hypothesis pipeline", disabled=not terms):
            try:
                payload = {
                    # no per-term type hints from this quick-test form -- empty
                    # `types` means "no constraint", same as the old terms-only
                    # behaviour on the backend.
                    "entities_input": [{"mention": t, "types": []} for t in terms],
                    "per_type_limit": 10,
