...
ENTITY_EMB_PATH = "/storage/Arushi/090526_EvoAge/kg_formation/training_2/EvoAge_121_12M/model_output/Dismult/DistMult_Evoage_121_12M_2/Evoage_121_12M_DistMult_entity.npy"

RELATION_EMB_PATH = "/storage/Arushi/090526_EvoAge/kg_formation/training_2/EvoAge_121_12M/model_output/Dismult/DistMult_Evoage_121_12M_2/Evoage_121_12M_DistMult_relation.npy"
...
# ---- DistMult config (must match training) ----
# DistMult has no gamma/margin — score is the raw trilinear form sum(h * r * t).
# Relation embeddings are a diagonal vector per relation: shape (R, d), same dim as entities.
# -------------------------------------------------
...
def load_embeddings(ent_path: str, rel_path: str, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if rel_np.shape[1] != ent_np.shape[1]:
        raise ValueError(
            ...
            f"DistMult relation embeddings should be the same dim as entities "
            f"(diagonal vector, not a flattened matrix like RESCAL or half-dim like RotatE)."
        )
...
@torch.no_grad()
def distmult_score_all_relations(
    h_vec: torch.Tensor,   # (B, d)
    t_vec: torch.Tensor,   # (B, d)
    rel_vec: torch.Tensor, # (Rc, d)
) -> torch.Tensor:
    """
    Vectorized DistMult score for a batch of (h, ?, t) against a chunk of relations.
    score(h, r, t) = sum_i h_i * r_i * t_i
    Returns raw scores of shape (B, Rc); higher is better.
    """
    ht = h_vec * t_vec                          # (B, d) elementwise h*t, precompute once
    scores = ht @ rel_vec.t()                   # (B, d) @ (d, Rc) -> (B, Rc)
    return scores
...
def evaluate_relation_prediction(
    ...
    """
    Computes Hits@K and MRR for relation prediction (h, ?, t) using DistMult.
    ...
    for i in range(0, N, batch_size):
        ...
        for j in range(0, R, rel_chunk):
            ...
            scores = distmult_score_all_relations(h_vec, t_vec, rel_chunk_vec)  # (B, Rc)
