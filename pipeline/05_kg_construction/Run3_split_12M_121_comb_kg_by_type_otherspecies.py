"""
# nohup python Run3_split_12M_121_comb_kg_by_type_otherspecies.py > Run3_split_12M_121_comb_kg_by_type_otherspecies.log &

═══════════════════════════════════════════════════════════════════════════════
 OTHER SPECIES — KG SPLIT into Aging_specific & Biomedical
═══════════════════════════════════════════════════════════════════════════════
 For each species:
   • Reads only  *_ortho_1_to_one2one_plus_one2many.csv  files
...
for species, folder in SPECIES.items():
    ...
    print(f"  ortho_1_to_one2one_plus_one2many files found: {len(files)}")
    ...
    for src in tqdm(files, desc=f"  {species}", unit="file",
        ...
        try:
            ...
            # Relation subfolder name (e.g. 'Celegans_gene_biologicalprocess')
...
log_path = os.path.join(ROOT, '_OTHER_SPECIES_KG_SPLIT_LOG.csv')
