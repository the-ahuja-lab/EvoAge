"""
═══════════════════════════════════════════════════════════════════════════════
 SPECIES TRIPLES — 1-to-many ortholog KG
═══════════════════════════════════════════════════════════════════════════════
 Sources:
   • Human   : all relation folders under generalised/ (except OTHER_SPECIES)
   • Other   : *one2one_plus_one2many.csv files under OTHER_SPECIES/
 Output:
   making_species_assocaitedwithconnection/one2one_plus_one2many_ortholog/
...
OUT_DIR = '/storage/Arushi/090526_EvoAge/kg_formation/processed_data_relation_wise_merge/making_species_assocaitedwithconnection/one2one_plus_one2many_ortholog'
...
# ── Collect OTHER_SPECIES 1-to-many files ────────────────────────────────────
# ortho_files = glob.glob(os.path.join(SPECIES_ROOT, '**/*_ortho_1_to_many.csv'), recursive=True)

ortho_files = glob.glob(
    os.path.join(
        SPECIES_ROOT,
...
print(f"Ortho 1to1+1toMany : {len(ortho_files)}")
...
for species in SPECIES_LIST:
    ...
    # PyArrow write — faster than pandas .to_parquet()
    pq.write_table(pa.Table.from_pandas(df_species, preserve_index=False), out_path)
...
summary_df.to_csv(os.path.join(OUT_DIR, '1_to_many_SUMMARY.csv'), index=False)
