...
ENTITY_EMB_PATH = "/storage/Arushi/090526_EvoAge/kg_formation/training_2/Biomedical_121_12M/model_output/Rescal/RESCAL_Biomedical_121_12M_1/Biomedical_121_12M_RESCAL_entity.npy"

RELATION_EMB_PATH = "/storage/Arushi/090526_EvoAge/kg_formation/training_2/Biomedical_121_12M/model_output/Rescal/RESCAL_Biomedical_121_12M_1/Biomedical_121_12M_RESCAL_relation.npy"

TRIPLES_PATH     = "/storage/Arushi/090526_EvoAge/kg_formation/final_kg_building_2/building_biomedical_kg_new/Store_House/Biomedical_1to1_KG_test_10.txt"
...
def main():
    log_path = "Biomedical_121_12M_rescal_edgetype.log"   # change to your desired path, e.g. "/storage/Arushi/.../rescal_edgetype.log"

    with open(log_path, "w") as log_file:
        def log(msg):
            print(msg)            # still shows in terminal
            log_file.write(msg + "\n")
            log_file.flush()      # write immediately, useful if the job crashes partway
        ...
        log(f"📦 Entity emb shape: {tuple(ent.shape)}")
        log(f"📦 Relation emb shape: {tuple(rel.shape)} (R, d, d)")
        log(f"🖥️ Device: {device} | batch={BATCH_SIZE} | rel_chunk={REL_CHUNK}")
        ...
        log(f"Total Triples Evaluated: {metrics['total']}")
        log(f"MRR     = {metrics['mrr']:.4f}")
        log(f"Hits@1  = {metrics['hits@1']:.4f}")
        log(f"Hits@3  = {metrics['hits@3']:.4f}")
        log(f"Hits@10 = {metrics['hits@10']:.4f}")
